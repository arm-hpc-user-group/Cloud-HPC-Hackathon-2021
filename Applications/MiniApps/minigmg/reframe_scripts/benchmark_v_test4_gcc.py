...
@rfm.simple_test
class MiniGMGvTest4GCC(hack.HackathonBase): ###################################################
    ...
    log_test_name = 'minigmg_validation_test4_gcc' ############################################
    ...
    @sn.sanity_function
    def get_time_in_sec_list(self, hour_list, min_list, sec_list):
        return list(z * 3600 + x * 60 + y
                    for z, x, y in sn.zip(hour_list, min_list, sec_list))
    ...
    def set_sanity_patterns(self):
        ...
        perf_regex_elapsed = r'(\S+)user\s+(\S+)system\s+(\S+):(\S+):(\S+)elapsed\s+(\S+)\s+(\S+)'

        elapsed_hour_list = sn.extractall(perf_regex_elapsed, self.stderr, 3, float)
        elapsed_min_list = sn.extractall(perf_regex_elapsed, self.stderr, 4, float)
        elapsed_sec_list = sn.extractall(perf_regex_elapsed, self.stderr, 5, float)

        time_in_sec_list = self.get_time_in_sec_list(elapsed_hour_list, elapsed_min_list, elapsed_sec_list)
