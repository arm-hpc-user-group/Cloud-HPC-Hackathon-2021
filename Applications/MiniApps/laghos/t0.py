...
# this test is number 1 in the verification table here: https://github.com/CEED/Laghos#verification-of-results
@rfm.simple_test
class LaghosTest0(hack.HackathonBase):
    ...
    executable_opts = ["-p", "0", "-dim", "2", "-rs",
                       "3", "-tf", "0.75", "-pa", ">", "laghos.out"]
    # executable_opts = []
    ...
    if run_mode in [RunMode.DEBUG_RUN, RunMode.PROFILE_RUN, RunMode.MATH_RUN, RunMode.PROF_MAP]:
        spec = parameter(['laghos@3.1 %arm'])
    elif run_mode == RunMode.CCOMP_RUN:
        spec = parameter(['laghos@3.1 %gcc /rpvemyc','laghos@3.1 %gcc /3a63qmj'],)
    else:
        spec = parameter([
            'laghos@3.1 %arm',  # arm
    ...
    elif run_mode in [RunMode.PROFILE_RUN, RunMode.PROF_MAP]:
        parallelism = parameter([{'nodes': 1, 'mpi': 1, 'omp': 1},  # for serial profiling
                                {'nodes': 1, 'mpi': 64, 'omp': 1}])  # for full node profiling
    elif run_mode in [RunMode.SSCALING_RUN,RunMode.CCOMP_RUN]:
    ...
    def set_sanity_patterns(self):
        ...
        self.sanity_patterns = sn.all([sn.assert_eq(lstep[0], 339), sn.assert_reference(
            lstep[2], 0.000702, -0.0001, 0.0001), sn.assert_reference(lstep[3], 4.9695537349e+01, -0.1, 0.1)])
